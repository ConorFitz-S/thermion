  ...
  test('load/clear skybox', () async {
    await ViewerBuilder(testHelper).setRenderTargetEnabled(true).execute((
      ...
      await testHelper.capture(
        result.viewer.view,
        "load_skybox_with_postprocessing",
      );
      await result.viewer.removeSkybox();
      await testHelper.capture(
        result.viewer.view,
        "remove_skybox_with_postprocessing",
      );
    });
  });

  test('transform gltf to unit cube', () async {
    await ViewerBuilder(testHelper).setCameraLookAt(Vector3(0, 0, 5)).execute((
      result,
    ) async {
      ...
      await asset.setTransform(
        Matrix4.compose(Vector3.zero(), Quaternion.identity(), Vector3.all(2)),
      );
